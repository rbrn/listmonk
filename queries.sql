...
WITH sub AS (
    ...
    ON CONFLICT (uuid) DO UPDATE SET status='blocklisted', updated_at=NOW()
    RETURNING id
)
...
SELECT  c.id, c.uuid, c.name, c.subject, c.from_email,
        c.messenger, c.started_at, c.to_send, c.sent, c.type,
        c.body, c.altbody, c.send_at, c.headers, c.status, c.content_type, c.tags,
        c.template_id, c.created_at, c.updated_at, c.userid, c.json,
        COUNT(*) OVER () AS total,
        (
            SELECT COALESCE(ARRAY_TO_JSON(ARRAY_AGG(l)), '[]') FROM (
                                                                        SELECT COALESCE(campaign_lists.list_id, 0) AS id,
...
-- name: get-campaign-sms-counts-by-userid
